    ...
    <div className="app-canvas text-content overflow-x-hidden">
      ...
      <header className="sticky top-0 z-40 backdrop-blur-md border-b border-contrast/10"
        ...
        <nav className="max-w-6xl mx-auto w-full px-4 sm:px-6 h-[62px] flex items-center justify-between gap-3">
          <div className="flex items-center gap-2.5 font-bold">
            <span className="w-7 h-7 rounded-lg grid place-items-center bg-gradient-to-b from-gold-bright to-gold text-on-gold shrink-0"><Spade size={15} /></span>
          ...
          <div className="flex items-center gap-4 sm:gap-5 text-sm text-content/70 shrink-0">
            ...
            {authed ? (
              ...
              <>
                ...
                <button onClick={startFree} className="rounded-lg px-3.5 sm:px-4 py-2.5 font-semibold bg-gradient-to-b from-gold-bright to-gold text-on-gold cursor-pointer whitespace-nowrap">{t('landing.nav.startFree')}</button>
              </>
            )}
          </div>
        </nav>
        ...
        <div className="relative z-[5] w-full intro-enter">
          <div className="max-w-6xl mx-auto px-6">
            ...
            <div className="mt-8 flex flex-wrap gap-3.5">
              <button onClick={startFree} className="rounded-xl px-6 py-3.5 font-semibold bg-gradient-to-b from-gold-bright to-gold text-on-gold cursor-pointer shadow-[0_8px_30px_-10px_rgba(212,168,71,.6)]">
          ...
          {STEPS.map((s, i) => (
            <Reveal key={s.n} delay={i * 0.06}>
              <div className="surface rounded-2xl p-6 h-full">
                <div className="w-7 h-7 rounded-lg grid place-items-center text-sm font-extrabold bg-gradient-to-b from-gold-bright to-gold text-on-gold mb-3.5">{s.n}</div>
      ...
      <section id="pricing" className="max-w-6xl mx-auto px-6 py-20">
        ...
        <Reveal delay={0.06} className="mt-10 grid gap-4 md:grid-cols-[1fr_1.15fr] items-stretch max-w-3xl">
          ...
          <div className="rounded-2xl p-7 flex flex-col relative border border-gold/50 bg-[linear-gradient(180deg,rgba(24,20,10,.55),var(--color-surface))] shadow-[0_0_0_1px_rgba(212,168,71,.15),0_40px_80px_-46px_rgba(212,168,71,.4)]">
            <div className="absolute -top-2.5 right-6 text-xs font-extrabold text-on-gold bg-gradient-to-b from-gold-bright to-gold px-3 py-1 rounded-full">{t('pricing.mostPopular')}</div>
            ...
            <div className="inline-flex mt-2 self-start bg-surface-2 border border-white/8 rounded-[11px] p-1 gap-1">
              <button onClick={() => setPlan('yearly')} className={`px-4 py-1.5 rounded-lg text-sm font-semibold cursor-pointer ${plan === 'yearly' ? 'bg-gradient-to-b from-gold-bright to-gold text-on-gold' : 'text-content/60'}`}>{t('pricing.yearly')}</button>
              <button onClick={() => setPlan('monthly')} className={`px-4 py-1.5 rounded-lg text-sm font-semibold cursor-pointer ${plan === 'monthly' ? 'bg-gradient-to-b from-gold-bright to-gold text-on-gold' : 'text-content/60'}`}>{t('pricing.monthly')}</button>
            </div>
            <div className="mt-5 flex flex-col gap-2.5 text-sm text-content/60">
            ...
            <button
              onClick={goPro}
              disabled={busy}
              className="mt-6 rounded-xl px-5 py-3 font-semibold bg-gradient-to-b from-gold-bright to-gold
      ...
      <section className="max-w-6xl mx-auto px-6 pb-20">
        <Reveal className="rounded-3xl border border-white/8 py-14 px-6 text-center bg-[radial-gradient(80%_120%_at_50%_0%,rgba(212,168,71,.12),transparent_60%),var(--color-surface)]">
          ...
          <div className="mt-7 flex justify-center gap-3.5 flex-wrap">
            <button onClick={startFree} className="rounded-xl px-6 py-3.5 font-semibold bg-gradient-to-b from-gold-bright to-gold text-on-gold cursor-pointer">{authed ? t('landing.hero.ctaOpen') : t('landing.hero.ctaStart')}</button>
      ...
      <footer className="border-t border-white/8 py-8">
        ...
        <div className="max-w-6xl mx-auto px-6 flex flex-wrap justify-between items-center gap-4 text-sm text-content/60">
          <div className="flex items-center gap-2.5 font-semibold text-content/80"><span className="w-6 h-6 rounded-md grid place-items-center bg-gradient-to-b from-gold-bright to-gold text-on-gold"><Spade size={13} /></span> Blackjack Trainer</div>
