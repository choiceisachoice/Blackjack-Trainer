  ...
  return (
    <div className="flex flex-col items-end gap-1">
      <button
        ...
        className="text-xs px-2 py-1 rounded bg-contrast/10 hover:bg-contrast/20 text-content/70 transition-colors"
      >
      ...
      {showCount && (
        <div className="bg-black/50 backdrop-blur-sm rounded-lg px-3 py-2 text-right">
          <div className="text-xs text-content/50 mb-1">{countingSystemId}</div>
          <div className="flex gap-4 text-sm">
            <div>
              <span className="text-content/50">RC </span>
              <span className={`font-bold ${runningCount >= 0 ? 'text-success' : 'text-error'}`}>
                {runningCount >= 0 ? '+' : ''}{runningCount}
              </span>
            </div>
            <div>
              <span className="text-content/50">TC </span>
