...
export function AnalyticsDashboard() {
  ...
  const setFlashFocus = useAppStore(s => s.setFlashFocus)
              ...
              <Panel title={t('analytics.weakestHands')} note={t('analytics.note.misplayed')}>
                {derived.weakest.length > 0 ? (
                  <>
                    ...
                    {/* The list travels with the mode switch. Without it this
                        button opened the ordinary flashcards, which draw from
                        every hand — the one thing the reader came to fix
                        appeared perhaps once in twenty questions. */}
                    <button
                      onClick={() => {
                        setFlashFocus(derived.weakest.map(h => h.name))
                        setMode('deviationTraining')
                      }}
                      data-testid="drill-weakest-hands"
